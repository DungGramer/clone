...
export async function respondForProgressive(e: FetchEvent) {
  ...
  if (start === 0 && end === 1) {
    const match = e.request.url.match(/fileSize=(\d+)&mineType=([\w/]+)/);
    const fileSize = match && Number(match[1]);
    const mimeType = match && match[2];

    if (fileSize && mimeType) {
      return new Response(new Unit8Array(2).buffer, {
        status: 206,
...
async function requestPart(
  ...
  if (!e.clientId) return undefined;
